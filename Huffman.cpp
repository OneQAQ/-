...
void Init()
{
	...
	for(i=1;i<=n;++i)
		printf("%c:%s\n",HT[i].ch,HC[i]); 
	fp=fopen("D:\Huffman\hfmTree.txt","w");
	fprintf(fp,"%d",n);
...
void Encoding()
{
	...
	if(n==0)
	{
		fp=fopen("D:\Huffman\hfmTree.txt","r");
		fscanf(fp,"%d",&n);
		if(feof(fp)){
			printf("请先初始化哈夫曼树！\n");
		}
	...
	fp = fopen("D:\Huffman\ToBeTran.txt","r");
	printf("成功读取ToBeTran.txt\n");
	fp1 = fopen("D:\Huffman\CodeFile.txt","w");
	//fscanf(fp,"%c",&in);
	while(!feof(fp))
	{
		for(i=1;i<=n;++i){
...
void Decoding()
{
	...
	fp=fopen("D:\Huffman\CodeFile.txt","r");
	fp1=fopen("D:\Huffman\TextFile.txt","w");
...
void Print()
{
	...
	printf("代码文件为:\n");
	fp=fopen("D:\Huffman\CodeFile.txt","r");
	fp1=fopen("D:\Huffman\CodePrin.txt","w");
	fscanf(fp,"%c",&in);
	while(!feof(fp))
	{
		printf("%c",in);
...
{
	FILE *fp;
	fp=fopen("D:\Huffman\TreePrint.txt","w");
	int i;
	for(i=1;i<2*n;i++){
